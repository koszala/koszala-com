<script>
	...
	function windowResized() {
		if (window.innerWidth > 600) {
			photoMode = 'desktop';
			photosDesktop.style.display = 'block';
			photosMobile.style.display = 'none';
		} else {
			photoMode = 'mobile';
			photosMobile.style.display = 'block';
			photosDesktop.style.display = 'none';
		}
	}

	onMount(() => {
		photosMobile = root.querySelector('.photos-mobile');
		photosDesktop = root.querySelector('.photos-desktop');
		windowResized();
	});
</script>
...
<div bind:this={root} class="screen">
	...
	<div class="photos photos-desktop">
		{#each data.items as item, i}
			<div
				class="photo"
				class:active={activePhoto === i}
